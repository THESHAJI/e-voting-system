// ════════════════════════════════════════════════════════════════
//  EVoting V3.1 — College Portal JavaScript
...
const translations = {
    en: {
        college_title: "College Student Union Election",
        college_subtitle: "Campus Democracy & Representative Governance",
        ...
        welcome_student: "Welcome, Student Voter",
        connect_desc_student: "Connect your decentralized wallet to authenticate with your university registration details. Identity is verified via campus-linked OTP.",
        connect_metamask: "Connect MetaMask Wallet",
        student_auth: "Student Verification",
        student_auth_desc: "Enter your university register number and contact details to receive your one-time voting authorization code.",
        reg_no_label: "Register / Roll Number", phone_label: "Mobile Number",
        email_label: "College / Personal Email",
        ...
        college_footer_title: "Institutional E-Voting Portal",
    ...
    hi: {
        college_title: "कॉलेज छात्र संघ चुनाव",
        college_subtitle: "परिसर लोकतंत्र और प्रतिनिधि शासन",
        ...
        welcome_student: "स्वागत है, छात्र मतदाता",
        connect_desc_student: "अपने विश्वविद्यालय पंजीकरण विवरण के साथ प्रमाणित करने के लिए विकेंद्रीकृत वॉलेट कनेक्ट करें।",
        connect_metamask: "MetaMask वॉलेट कनेक्ट करें",
        student_auth: "छात्र सत्यापन",
        student_auth_desc: "अपना वन-टाइम वोटिंग कोड प्राप्त करने के लिए अपना विश्वविद्यालय रजिस्टर नंबर और मोबाइल नंबर दर्ज करें।",
        reg_no_label: "रजिस्टर / रोल नंबर", phone_label: "मोबाइल नंबर",
        email_label: "कॉलेज / व्यक्तिगत ईमेल",
        ...
        college_footer_title: "संस्थागत ई-मतदान पोर्टल",
    ...
    ta: {
        college_title: "கல்லூரி மாணவர் சங்க தேர்தல்",
        college_subtitle: "வளாக ஜனநாயகம் மற்றும் பிரதிநிதித்துவ ஆட்சி",
        ...
        welcome_student: "வணக்கம், மாணவர் வாக்காளரே",
        connect_desc_student: "உங்கள் பல்கலைக்கழக பதிவு விவரங்களுடன் அங்கீகரிக்க உங்கள் வாலட்டை இணைக்கவும்.",
        connect_metamask: "MetaMask வாலட்டை இணைக்கவும்",
        student_auth: "மாணவர் சரிபார்ப்பு",
        student_auth_desc: "உங்கள் வாக்களிப்பு அங்கீகார குறியீட்டைப் பெற உங்கள் பல்கலைக்கழக பதிவு எண் மற்றும் மொபைல் எண்ணை உள்ளிடவும்.",
        reg_no_label: "பதிவு / ரோல் எண்", phone_label: "மொபைல் எண்",
        email_label: "கல்லூரி / தனிப்பட்ட மின்னஞ்சல்",
        ...
        college_footer_title: "நிறுவன மின்-வாக்களிப்பு தளம்",
...
function showToast(msg, error = false) {
    ...
    toast.innerText = msg;
    toast.style.borderLeftColor = error ? "#ef4444" : "#3b82f6";
...
function enableInstantDemoStudent() {
    demoMode = true;
    ...
    status.innerHTML = `● Instant Student Wallet: ${currentWallet.slice(0,6)}...${currentWallet.slice(-4)}`;
    status.classList.remove("status-warning");
    status.classList.add("status-online");

    showToast("⚡ Instant Student Session Active (No Popups)");
    showSection("section2");
}
window.enableInstantDemoStudent = enableInstantDemoStudent;

// ═══════════════ 1. CONNECT WALLET ═══════════════
document.getElementById("connectBtn").onclick = async () => {
    try {
        if (demoMode || !window.ethereum) {
            enableInstantDemoStudent();
            return;
        }
...
document.getElementById("registerBtn").onclick = async () => {
    ...
    if (!registerNo || registerNo.length < 5) {
        return showToast("Register Number must be at least 5 characters", true);
    }
    if (!phone || !/^\d{10}$/.test(phone)) {
        return showToast("Phone must be exactly 10 digits", true);
    }
    ...
    try {
        const res = await fetch(`${backendUrl}/register-college`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ registerNo, phone, email })
        });
        const data = await res.json();

        if (res.ok) {
            currentRegHash = data.regHash;
...
async function loadCandidates() {
        ...
        try {
            ...
            for (let i = 0; i < total; i++) {
                ...
                card.innerHTML = `
                    ...
                    <div class="candidate-icon" ${logoHash ? 'style="display:none"' : ''}>🎓</div>
    ...
    try {
        ...
        if (parties.length === 0) {
            list.innerHTML = `<div style="grid-column: 1/-1; text-align:center; color: var(--text-secondary); padding: 2rem;">
                <p style="font-size: 1.2rem; margin-bottom: 0.5rem;">🎓</p>
                <p>No candidates registered yet. Election authority must add candidates.</p>
            </div>`;
            return;
        }
        ...
        parties.forEach((party, i) => {
            ...
            card.innerHTML = `
                ...
                <div class="candidate-icon" ${party.logo ? 'style="display:none"' : ''}>${party.symbol || '🎓'}</div>
    ...
    try {
        if (contract && !demoMode) {
            ...
            await fetch(`${backendUrl}/cast-vote`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    identityHash: currentRegHash || currentWallet,
                    partyId: index,
                    txHash: tx.hash,
                    type: "college"
                })
            ...
            showToast("✅ Student vote committed to blockchain!");
        } else {
            ...
            await fetch(`${backendUrl}/cast-vote`, {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    identityHash: currentRegHash || currentWallet || "demo_student",
                    partyId: index,
                    txHash: fakeHash,
                    type: "college"
                })
...
function storeSecret(wallet, secret, candidateIndex, candidateName, txHash) {
    const vault = JSON.parse(localStorage.getItem("evoting_vault_college") || "{}");
    vault[wallet] = {
        secret,
        candidateIndex,
        candidateName,
    ...
    localStorage.setItem("evoting_vault_college", JSON.stringify(vault));
...
function nextVoter() {
    ...
    showToast("Ready for next student voter!");
}
...
async function revealVote() {
    ...
    try {
        if (contract && !demoMode) {
            ...
            showToast("✅ Vote counted on the ledger!");
        } else {
...
async function loadResults() {
    ...
    candidates.forEach(c => {
        ...
        card.innerHTML = `
            ...
            <div class="candidate-icon" ${c.logo ? 'style="display:none"' : ''}>${c.symbol || '🎓'}</div>
...
═══════════════════════════════════════════
     ...
     College E-Voting System V3.1
═══════════════════════════════════════════

Election:     COLLEGE / INSTITUTIONAL
...
═══════════════════════════════════════════
    ...
    a.href = url;
    a.download = `college_vote_receipt_${Date.now()}.txt`;
    a.click();
